...
# 如果Bastion主機的Openshift使用者不是$openshift_cred中輸入的, 會透過plink做oc login
if (Invoke-Expression "$bastion_remote_control 'oc whoami 2>/dev/null' | findstr $openshift_cred.UserName" ){
...
# 與Jenkins API連線取得套件清單
Invoke-Expression "$bastion_remote_control 'curl -s --header 'Authorization: Bearer $(oc whoami -t)' -k https://$($jenkins_url)/pluginManager/api/xml\?depth\=1'" > jenkins_plugin_list.xml

# 傳輸套件清單與執行Python腳本下載套件
